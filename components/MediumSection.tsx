          ...
          <div className="grid gap-4 md:grid-cols-2">
            {posts.length === 0
              ? Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-40 animate-pulse rounded-xl border border-border/40 bg-muted/20"
                  />
                ))
              : posts.map((post, index) => (
                  <motion.a
                    ...
                    className="group relative overflow-hidden rounded-xl border border-primary/20 bg-gradient-to-br from-card/90 via-card/50 to-card/30 p-5 shadow-[0_0_40px_-18px_oklch(0.78_0.14_195/0.35)] transition hover:border-primary/45 hover:shadow-[0_0_50px_-12px_oklch(0.78_0.14_195/0.45)]"
                  >
